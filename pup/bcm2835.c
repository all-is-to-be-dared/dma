...
void *xz_malloc_stub(size_t size) {
...
bool platform_init_meta(meta_t *meta) {
  ...
  if (meta->compression_type >= NUM_COMPRESS_TYPES) {
    printf(BOOT FUNC("platform_init_meta") ERROR
           "unknown compression type %" PRId32 "\n",
           meta->compression_type);
    return false;
  }
  if (meta->image_format >= NUM_IMAGE_FORMATS) {
    printf(BOOT FUNC("platform_init_meta") ERROR "unknown image format %" PRId32
                                                 "\n",
           meta->image_format);
    return false;
  }
  ...
  case IMAGE_FLAT_BINARY:
    flat_binary_load_addr =
        meta->format_metadata.flat_binary_metadata.load_addr;
    if (flat_binary_load_addr >= DRAM_LIMIT) {
      printf(BOOT FUNC("platform_init_meta") ERROR
             "load address outside of address space\n");
      return false;
    }
    if (flat_binary_load_addr + (uint64_t)meta->mem_size >= DRAM_LIMIT) {
      printf(BOOT FUNC("platform_init_meta") ERROR
             "image size overflows address space\n");
      return false;
    }
    ...
    break;
  case IMAGE_ELF:
    // okay, this is more complicated. basically, we need to just pick a spot in
    // memory to load the image; when image loading is done, parse the ELF and
    // do a second-stage loading process
  ...
  printf(BOOT FUNC("platform_init_meta") "image = [%" PRIxPTR ",%" PRIxPTR
                                         ")\n",
         img_start, img_end);
  printf(BOOT FUNC("platform_init_meta") "prog  = [%" PRIxPTR ",%" PRIxPTR
                                         ")\n",
         prog_start, prog_end);
  ...
  if (reloc_size) {
    ...
    printf(BOOT FUNC("platform_init_meta") "relocate [%x,%x] to [%x,%x] (%zx "
                                           "bytes), stub=[%x,%x]\n",
           reloc_tgt_start, reloc_tgt_end, reloc_buf_start, reloc_buf_end,
           reloc_size, stub_start, stub_end);
...
static void feed_impl(uint8_t *data, size_t len) {
  size_t cnt, off;
...
bool platform_feed(size_t chunk_no, uint8_t *data, size_t len) {
  struct xz_buf xzbuf;
  enum xz_ret r;
  size_t prev_in_pos;
    ...
    while (xzbuf.in_pos < xzbuf.in_size) {
      ...
      case XZ_STREAM_END:
        printf(BOOT FUNC("platform_feed") "inflated %d->%d bytes\n",
               xzbuf.in_pos - prev_in_pos, xzbuf.out_pos);
      ...
      case XZ_BUF_ERROR:
        ...
        return false;
      default:
        printf(BOOT FUNC("platform_feed") ERROR
               "xz_dec_run failed with unknown code %d\n",
               r);
        return false;
      }
    }
...
bool platform_load_image(void) {
  ...
  if (feed != end) {
    printf(BOOT FUNC("platform_load_image") ERROR
           "feed (%p) did not reach end of image (%p)\n",
           feed, end);
    return false;
  }
  ...
  case COMPRESS_NONE:
    ...
    if (!crc_ok) {
      printf(BOOT FUNC("platform_load_image") ERROR "CRC: expected %" PRIx32
                                                    " got %" PRIx32 "\n",
             META.mem_crc32, crc);
    }
    break;
  ...
  case IMAGE_ELF:
    format_ok =
        load_elf_image(img_start, img_end, true, true, EM_ARM, 0x2000'0000, "");
    break;
  default:
