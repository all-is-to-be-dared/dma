...
static bool
find_trace32_sections(void *elf_image_base, struct trace32_sections *sections)
{
  Elf32_Ehdr *ehdr;
...
static void dump_symbol_table_elf32(struct trace32_sections *sections)
{
  ...
  assert(sections->symtab->sh_entsize, "SHT_SYMTAB should be a table of fixed-size entries\n");
  assert(sizeof *sym == sections->symtab->sh_entsize, "SHT_SYMTAB entries have unexpected size\n");

  assert((sections->symtab->sh_size % sections->symtab->sh_entsize) == 0, "SHT_SYMTAB size is not a multiple of %d\n", sizeof *sym);
  ...
  for(i = 0;i < cnt;i++) {
    ...
    printf("Symbol #%d: %s (%s) %s '%s' = %p <%zu>\n", i, pub, binds, typs, 
      sym->st_name ? strtab + sym->st_name : "<UNNAMED>", (void*)sym->st_value, sym->st_size);
    sym += 1;
  }
}
static const char *backtrace_get_symbol_elf32(struct trace32_sections *sections, uintptr_t pc)
{
  Elf32_Sym *sym;
  const char *strtab;
  uint32_t i, cnt;
...
bool backtrace_enable(void *elf_image_base) {
  ...
  // Currently, the only backtrace supported is the frame-pointer based one that works off the
  // symbol table. However, this only works if the compiler has been told not to omit frame pointer
  // codegen. Thus: we check that frame-pointer is not omitted!
...
backtrace_cursor backtrace_cursor_new()
{
  ...
  ra = (uintptr_t)__builtin_extract_return_addr(__builtin_return_address(0)) - 4;
  sframe = __builtin_frame_address(0);
  return (backtrace_cursor){ .pc = ra, .fp = sframe->fp };
}

void backtrace_cursor_read(backtrace_cursor *curs, uintptr_t *pc, const char **sym)
{
