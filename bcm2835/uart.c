...
static uint16_t baud_to_prescaler(uint32_t baud_rate, uint32_t core_freq)
{
  return (uint16_t)((core_freq / (8u * baud_rate)) - 1u);
}

void aux_uart_init(uint32_t baud_rate, uint32_t core_freq) {
  dsb();
...
void aux_uart_clear_fifos(void) {
  dsb();
  aux_uart->iir = AUX_UART_IIR_CLEAR_BOTH;
}

void aux_uart_set_baud_rate(uint32_t baud_rate, uint32_t core_freq) {
  dsb();
  aux_uart_clear_fifos();
  aux_uart->cntl &= ~(AUX_UART_CNTL_TX_ENABLE | AUX_UART_CNTL_RX_ENABLE);
  aux_uart->baud = (uint32_t)baud_to_prescaler(baud_rate, core_freq);
...
bool aux_uart_tx_idle(void) {
  bool r = (aux_uart->stat & AUX_UART_STAT_TX_DONE) != 0;
  dsb();
  return r;
}

void aux_uart_flush_tx_fifo(void) {
  while (!aux_uart_tx_idle())
    ;
}

bool aux_uart_can_put(void) {
