...
main(struct elf_boot_args *boot_args)
{
  ...
  for (i = 0; i < N; i++) {
    ...
    // printf(
    //   "read %#x bytes in %dcy / %llu.%03llums : %f MB/s\n", ep, c, dus / 1000, dus % 1000, tput);
    printf("Reading... [%d/%d]\r", i, N);
    s += tput;
    avg_cy_read += c;
  }
  printf("Reading... [%d/%d]\n", N, N);
  avg_tput_read = s / (double)N;
  avg_cy_read /= N;
  ...
  printf("Hashing... [%d/%d]\n", N, N);
  avg_tput_hash = s / (double)N;
  avg_cy_hash /= N;

  printf("Mean read throughput: %f MB/s\n", avg_tput_read);
  printf("Mean hash throughput: %f MB/s\n", avg_tput_hash);
  printf("Delta: %f MB/s\n", avg_tput_hash - avg_tput_read);
  printf("Mean read latency: %lld cy\n", avg_cy_read);
  printf("Mean hash latency: %lld cy\n", avg_cy_hash);
  printf("Delta: %lld cy, %f cycles/32B\n",
         avg_cy_hash - avg_cy_read,
         (double)(avg_cy_hash - avg_cy_read) / (double)(ep / 32));
...
//   // Modified version of XXH32, designed for maximum throughput.
//   //
//   // The key characteristic is the width of the load: the loop loads 32 bytes at a time.
//   // This burns 8 registers, which forces us down to only 3 stripes for the hasher.
//   {
//     register uint32_t _s0 asm("r0") = s1, _s1 asm("r1") = s1, _s2 asm("r2") = s2;
//     register const uint8_t *_data asm("r3") = data;
//     register uint32_t prime1 asm("r4") = XXH_PRIME_1, prime2 asm("r5") = XXH_PRIME_2;
//     register const uint8_t *stop asm("r14") = stop_block;
//     __asm__("push {r4-r12, r14}\n"
//             "adr r7, 3f\n"
//             "str sp, [r7]\n"
//             "mov r12, r4\n"
//             "mov r13, r5\n"
